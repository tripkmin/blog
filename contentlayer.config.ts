...
import rehypeCodeTitles from 'rehype-code-titles'; // 코드 블록에 타이틀을 추가해주는 라이브러리
import rehypePrism from 'rehype-prism-plus'; // Syntax Highlighting을 위한 라이브러리
      ...
      resolve: async post => {
        ...
        const headings = Array.from(post.body.raw.matchAll(regXHeader)).map(
          ({ groups }) => {
            ...
            return {
              level: flag?.length, // 제목 레벨(heading level) 계산
              text: content, // 제목 내용(heading text)
              slug: content ? slugger.slug(content) : undefined, // 제목의 슬러그(slug) 생성
            };
          }
        );
        return headings; // 계산된 제목들 반환
      },
    },
  },
}));
...
export default makeSource({
  ...
  documentTypes: [Post, Project], // 사용할 문서 유형 목록
  mdx: {
    remarkPlugins: [remarkBreaks, remarkToc, remarkGfm], // remark 플러그인 목록
    rehypePlugins: [
      rehypeSlug, // 슬러그(slug) 생성을 위한 플러그인
      rehypeCodeTitles, // 코드 블록에 타이틀 추가를 위한 플러그인
      rehypePrism, // Syntax Highlighting을 위한 플러그인
      [
